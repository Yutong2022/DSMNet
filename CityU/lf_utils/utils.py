...
def cal_ssim(img1, img2):
    return metrics.structural_similarity(img1, img2, gaussian_weights=True)

def cal_metrics(img1, img2, angRes):

    PSNR = np.zeros(shape=(angRes*angRes), dtype='float32')
    SSIM = np.zeros(shape=(angRes*angRes), dtype='float32')

    for i in range(angRes*angRes):
        PSNR[i] = cal_psnr(img1[0, i], img2[0, i])
        SSIM[i] = cal_ssim(img1[0, i], img2[0, i])
        pass
