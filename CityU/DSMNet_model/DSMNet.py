...
class get_model(nn.Module):
    ...
    def forward(self, lf_lr,  Lr_Info = None):
        ...
        lf_lr = einops.rearrange(lf_lr, 'b (u v) h w -> b 1 (u h) (v w)', u=self.an, v=self.an)
        ...
        out = einops.rearrange(out, 'N 1 (u H) (v W) -> N (1 u v) H W', u=self.an, v=self.an)
...
class get_loss(nn.Module):
    ...
    def forward(self, SR, HR):
        ...
        # yv
        SR_eyv = einops.rearrange(SR, 'b (c a1 a2) h w -> (b h a1) c w a2', a1=self.angRes, a2=self.angRes)
        HR_eyv = einops.rearrange(HR, 'b (c a1 a2) h w -> (b h a1) c w a2', a1=self.angRes, a2=self.angRes)
        ...
        # hu
        SR_ehu = einops.rearrange(SR, 'b (c a1 a2) h w -> (b w a2) c h a1', a1=self.angRes, a2=self.angRes)
        HR_ehu = einops.rearrange(HR, 'b (c a1 a2) h w -> (b w a2) c h a1', a1=self.angRes, a2=self.angRes)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--scale_factor", type=int, default=4, help="Learning rate")
    parser.add_argument("--angRes_in", type=int, default=5, help="Learning rate decay every n epochs")
    parser.add_argument("--angRes", type=int, default=5, help="Learning rate decay every n epochs")
    ...
    # inference the model
    x = torch.randn(b, c, u*h, v*w)
    net = get_model(args=opt)
    net = net.to('cuda')
    x = x.to('cuda')
    x_hr = torch.randn(b, c*u*v, 4*h, 4*w).to('cuda')
    criterion_Loss = get_loss(opt).to('cuda')
    print(sum([p.numel() for p in net.parameters()])/(1024*1024))
